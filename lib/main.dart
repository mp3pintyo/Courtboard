                  ...
                  content: SizedBox(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        DropdownButtonFormField<String>(
                            ...
                            items: const ['NBA', 'WNBA', 'Foci', 'Darts', 'NFL']
...
class _ComparePage extends StatefulWidget {
  const _ComparePage({required this.athletes});
  final List<Athlete> athletes;
  @override
...
class _ComparePageState extends State<_ComparePage> {
  late Athlete left = widget.athletes.first;
  late Athlete right =
      widget.athletes.length > 1 ? widget.athletes[1] : widget.athletes.first;

  @override
  Widget build(BuildContext context) {
    final rows = List.generate(
        4, (index) => (left.metrics[index], right.metrics[index]));
    return Container(
      color: _canvas,
      padding: const EdgeInsets.all(34),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Összehasonlítás',
            style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900)),
        const SizedBox(height: 6),
        const Text('Azonos sportágban a mezők közvetlenül összevethetők.',
            style: TextStyle(color: _muted)),
        const SizedBox(height: 24),
        Row(children: [
          Expanded(
              child: DropdownButtonFormField<Athlete>(
                  initialValue: left,
                          ...
                          Expanded(
                              child: Text(row.$1.value,
                                  ...
                                  style: const TextStyle(
                                      ...
                                      fontWeight: FontWeight.w900))),
                          Expanded(
                              child: Text(row.$1.label,
                                  textAlign: TextAlign.center,
                                  style: const TextStyle(
                                      color: _muted,
                                      fontWeight: FontWeight.w800))),
                          Expanded(
                              child: Text(row.$2.value,
                                  style: const TextStyle(
                                      fontSize: 22,
                                      fontWeight: FontWeight.w900)))
                        ])))
